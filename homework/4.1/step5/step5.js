...
angular.module('PokemonApp', [])
    .controller('PokemonListCtrl', function ($scope, $window) {
        ...
        // получаем массив типов покемонов для дропдауна
        $scope.getPokemonsTypes = function (pokemonsArr) {
            const len = pokemonsArr.length;
            let arr = [];

            for (var i=0; i<len; i++) {
              arr = arr.concat(pokemonsArr[i].type);
            };

            // удаляем из массива дупликаты
            const arrNoDupes = Array.from(new Set(arr));

            return arrNoDupes;
        };

        $scope.pokemonsTypes = $scope.getPokemonsTypes($scope.pokemons);

        $scope.selectedPokemonType = "normal";

        // вывод alert для п.4.
        $scope.doAlert = function (alertMsg) {
